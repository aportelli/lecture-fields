...
\paragraph{Proof of (6):} We can write
\begin{eqnarray*}
  \Div \left(\v{a}\times\v{b}\right)
  ~ = ~
...
\paragraph{Example:}
Show that $\Div \left(|\vr|^{-3}\vr\right) = 0$, for $|\vr|\ne 0$.

\paragraph{Method~1:} Using identities and simple results: Using identity~(3), we have
\[
  \Div \left(|\vr|^{-3}\vr\right)
  ~=~
  \left(\grad |\vr|^{-3}\right) \cdot \vr
  ~+~
  |\vr|^{-3}\; \left(\Div \vr\right)
\]

But we've shown previously that $\grad |\vr|^n = n\,|\vr|^{n-2}\,\vr$ and $\Div\vr = 3$.
Hence
\begin{eqnarray*}
  \Div \left(|\vr|^{-3}\vr\right)
  %  & = &
  ...
  & = &
  \left(\frac{-3}{|\vr|^5}\, \vr\right) \cdot \vr
  \,+\,
  \frac{3}{|\vr|^3}\\[1ex]
  & = &
  \frac{-3}{|\vr|^5} |\vr|^2 \,+\, \frac{3}{|\vr|^3}
  ~=~ 0
\end{eqnarray*}
...
\begin{eqnarray*}
  \Div \left(|\vr|^{-3}\vr\right)
  =
  \partial_i \left(x_i/|\vr|^3\right)
  &=&
  \left(\partial_i x_i\right)/|\vr|^3 + x_i \, \partial_i \, |\vr|^{-3} \\
  &=&
  \delta_{ii}/|\vr|^3 + x_i \left(-3 x_i / |\vr|^5\right) \\
  &=&
  3/|\vr|^3 - 3/|\vr|^3
  ~=~
  0
...
\subsection{Identities involving two \texorpdfstring{$\grad$}{∇}s}
...
\end{eqnarray*}
since any vector equal to minus itself is must be zero.  The proof of (9) is similar. It
is important to understand how these two identities stem from the anti-symmetry of
$\eps{ijk}$. Identity~(10) can be proven using the identity for the product of two epsilon
symbols (see exercises). Again, the proof is far simpler than trying to use \emph{`xyz'}.
It is an extremely important result and is used frequently in electromagnetism, fluid
mechanics, and other `field theories'. Identity~(10) is also used in curvilinear
coordinate systems to \emph{define} the action of the Laplacian on a vector field as
\[
  \lap\,\v{a}
  ~\equiv~
...
\[
  \grad\phi(r)
  \,=\,
  \frac{\diff\phi(r)}{\diff r} \, \grad r
  \,=\,
  \phi'(r) \, \frac{\,\v{r}\,}{r},
\]
and is left to the tutorial.

\paragraph{Example:} Evaluate $\curl\left\{\left( \v{c}\times\v{r} \right) /|\vr|^3\right\}$ where $\v{c}$ is
a constant vector.

\medskip

Start with the product rule \( \curl (\phi\,\v{a}) \,=\, (\grad\phi)\times\v{a} \,+\,
\phi\left(\curl \v{a}\right) \) where $\phi=1/|\vr|^3$ and $\v{a}=\v{c}\times\v{r}$.
\begin{eqnarray*}
  \curl\left( \frac{\v{c}\times\v{r}}{|\vr|^3} \right)
  & = &
  \grad\left(\frac{1}{|\vr|^3}\right) \times \left(\v{c}\times\v{r}\right)
  \,+\,
  \frac{1}{|\vr|^3} \, \curl \left(\v{c}\times\v{r}\right) \\[1ex]
  & = &
  -3 \, \frac{\,\v{r}\,}{|\vr|^5} \times \left(\v{c}\times\v{r}\right)
  \,+\,
  \frac{1}{|\vr|^3} \, 2\v{c} \\[1ex]
  & = &
  -3\,\frac{1}{|\vr|^5}
  \left( |\vr|^2 \v{c} - \left(\v{r}\cdot\v{c}\right)\v{r}\right)
  +
  \frac{2}{|\vr|^3} \, \v{c} \\[1ex]
  & = &
  \frac{3\,\left(\v{r}\cdot\v{c}\right)\v{r}}{|\vr|^5}
  \,-\,
  \frac{\,\v{c}\,}{|\vr|^3}
\end{eqnarray*}
where we used $\grad |\vr|^n = n |\vr|^{n-2} \v{r}$ with $n=-3$, and $\curl
  \left(\v{c}\times\v{r}\right) = 2 \v{c}$. A useful exercise left to the reader is to do
the entire calculation using suffix notation.

\paragraph{Physical example:} The vector potential of a magnetic dipole has the form $\left( \v{c}\times\v{r} \right)
  /|\vr|^3$.
...
\[
  \begin{array}{ll ll ll l}
    \grad |\vr| \,=\, \v{r}/|\vr| \,=\, \v{\hat{r}}
     & \, &
    \grad |\vr|^n \,=\, n |\vr|^{n-2} \v{r}
     & \, &
    \Div \v{r} \,=\, 3
     & \, &
    \curl \v{r}\,=\,0 \\[1.5ex]
  ...
  \end{array}
\]
where $\v{r}$ is the position vector, and $\v{a}$ is a \emph{constant} vector. The
identity $\left(\v{a} \cdot \grad\right)\v{r} = \v{a}$ holds also for vector
\emph{fields} $\v{a}(\v{r})$, because no derivatives act on $\v{a}$.

\paragraph{Identities for scalar fields:}
You must know, and be able to derive, all the following for scalar fields $\phi(\v{r})$
and $\psi(\v{r})$ using index notation:
...
\begin{enumerate}
  ...
  \item Chain rule:\ \ If $F[\phi(\vr)]$ is a scalar field, then $\ds \grad F\left(\phi\right)
          \,=\, F'[\phi(\v{r})]\; \grad\phi(\v{r}) $

        \emph{Important example:}\ \ $\grad f(|\v{r}|) \,=\, [f'(|\v{r}|)/|\v{r}|] \v{r}$
\end{enumerate}
