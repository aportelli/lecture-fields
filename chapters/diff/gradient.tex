...
surfaces do not intersect, or $\phi$ would be multivalued at the point of intersection.
...
Isobars on a weather map are level curves of pressure $p(x_1,x_2)=c$. In physics, when
the field is a scalar potential (see later) the level surfaces are known as
\emph{equipotentials}.
%
...
\begin{enumerate}
  \begin{figure}[t]
    ...
    \caption{Representation of the equipotentials of a Coulomb potential from a source with position vector $\v{a}$.}
    \label{fig:coulomb_pot}
  \end{figure}
  \item Let \( \; \phi(\v{r}) = \v{r}^2 \equiv x_1^2+x_2^2+x_3^2
        %\equiv r_1^2 + r_2^2 + r_3^2
        \).
        The level surface $\phi(\v{r}) = r_0^2$ is a sphere of radius $r_0$ centred on the
        origin. As $r_0$ is varied, we obtain a family of level surfaces or equipotentials which are
        concentric spheres.

  \item The electrostatic Coulomb potential at $\v{r}$ due to a point charge $q$ situated at the
        point $\v{a}$ is
        \[
        ...
        spheres centred on the point $\v{a}$, as shown in \cref{fig:coulomb_pot}. If $\v{a}=0$,
        the equipotentials are centred on the origin.
...
position? As an example, think of a two-dimensional contour map of the height
$h=h(x_1,x_2)$ of a hill, which is a scalar field. If we are on the hill and move in the
$(x_1,x_2)$ plane then the change in height will depend on the direction in which we move
(unless the hill is completely flat!). For example there will be a direction in which the
height increases most steeply: ``straight up the hill''. We now introduce a formalism to
describe how a scalar field $\phir$ changes as a function of position $\v{r}$ based on
this intuition. We begin by recalling Taylor's theorem and the definition of partial
derivatives.
...
\[
  ...
  f(x)
  +
  \delta x \, \frac{\diff f(x)}{dx}
  +
  \frac{(\delta x)^2}{2!} \, \frac{\diff ^2f(x)}{dx^2}
  + \cdots +
  \frac{(\delta x)^n}{n!}  \, \frac{\diff ^nf(x)}{dx^n}
  + \cdots
\]
If $\delta x$ is very small, we may approximate\ \ \( \displaystyle f(x+\delta x) = f(x)
+ \delta x \, \frac{\diff f(x)}{dx} + O[(\delta x)^2] \)
...
\[
  \frac{\partial f(x_1,x_2,x_3)}{\partial x_1}=\lim_{\delta x\to 0}\frac{f(x_1+\delta x,x_2,x_3)-f(x_1,x_2,x_3)}{\delta x}\,,
\]
if it exists, is obtained by differentiating $f(x_1,x_2,x_3)$ with respect to $x_1$,
whilst keeping $x_2$ and $x_3$ \emph{fixed}. The partial derivatives with respect to
$x_2$ and $x_3$ can be similarly defined.
...
\begin{figure}[t]
  ...
  \caption{Small displacement between two level surfaces of a scalar field.}
  \label{fig:level_1}
\end{figure}
...
\emph{different} level surfaces as shown in \cref{fig:level_1}.
Now one can use Taylor's theorem to first order in each of the 3 variables $x_1$, $x_2$ and $x_3$
...
the dummy index $i$. The vector field $\grad\phi(\v{r})$ is called the \emph{gradient} of
$\phi(\v{r})$. It is sometimes pronounced ``grad phi'', and the character $\nabla$ is
called ``nabla''.

Let us look at a few examples in the full $(x_1$, $x_2$, $x_3)$ notation before switching
to suffix notation with the summation convention.

\paragraph{Example:} Calculate the gradient of the scalar field $\ds \phi(\v{r}) = \v{r}^2 = x_1^2 + x_2^2 +
  x_3^2\,.$ First, recall that the \emph{partial derivative} $\partial\phi/\partial \x{1}$
is the derivative of $\phi(\v{r})=\phi(x_1,x_2,x_2)$ with respect to $x_1$, keeping $x_2$
and $x_3$ \emph{fixed}, \emph{etc}.
\[
  \mbox{So} \quad
  ...
  \quad \mbox{etc.}
\]
The first component of $\grad\v{r}^2$ is then
\[
...
\[
  ...
  2\v{r}
\]
The vector $\grad\v{r}^2 = 2\v{r}$ points radially outwards from the origin with
\emph{magnitude} $2|\v{r}|$. The level surfaces, $\v{r}^2=\mbox{constant}$, are spheres
...
\[
  ...
  \v{a}
\]
This is a very important result -- as we shall see later.

\noindent\textbf{NB:}
An important result for partial derivatives is
\[
  \frac{\partial{x_i}}{\partial{x_j}} = \partial_ix_j=\delta_{ij}
...
\]
The infinitesimal vector $\delta\v{r}$ lies in the level surface at $\v{r}$, and the
above equation holds for all such $\delta\v{r}$, hence
\begin{center}
  \bigbox{
    \vspace*{-2ex}
    \parbox{65mm}{
...
\[
  \vn(\v{r}) ~=~ \frac{\grad\phi(\v{r})}{|\grad\phi(\v{r})|}
  \qquad
  \left(\mbox{for} \quad |\grad\phi(\v{r})| \neq 0\right)
\]
...
$\v{\hat{s}}$ is given by
\begin{equation}
  \nabla_{\hat{\v{s}}}\,\phi=\v{\hat{s}} \cdot \grad\phi
  =
  |\grad\phi| \, \cos(\theta)
  \label{eq:dir-deriv}
\end{equation}
...
\begin{center}
  \bigbox{
    $\nabla_{\hat{\v{s}}}\,\phi$ is called the \emph{directional
...
when $\v{\hat{s}}$ is tangent to the level surface. Therefore,
\begin{center}
...
through the atmosphere with velocity $\v{v}$. Obtain an expression for the rate of change
of temperature experienced by the object. As the object moves from $\v{r}$ to
$\v{r}+\delta\v{r}$ in time $\delta t$, it sees a change in temperature
\[
  \delta T
...
\[
  \frac{\diff T(\v{r})}{\diff t} =   \vv \cdot \grad T(\v{r})
\]
...
\begin{center}
  \bigbox{$  \v{F} = -\grad V $}
\end{center}
where $V$ is the potential energy. Such a force is called a {\em conservative} since, as
we shall prove later on, Newton's law then implies that the total energy (kinetic plus
potential) is conserved.

\paragraph{Gravitational force due to the Earth:}
The potential energy of a particle of mass $m$ at a height $z$ above the Earth's surface
is $V = mgz$. The force due to gravity can be written as
\[
  \v{F} = -\grad V = -mg \, \ethree
\]

\paragraph{Newton's Law of Gravitation:}
Now consider the gravitational force on a mass $m$ at $\vr$ due to a mass $m_0$ at the
origin. We can write this as
\[
  \v{F}
  \,=\,
  -\frac{G m m_0}{\v{r}^2} \: \v{\hat{r}}
  \,=\,
  - \grad V
\]
where the potential energy $V = -Gm m_0/|\v{r}|$ and $\v{\hat{r}}$ is a unit vector in
the direction of $\v{r}$. We shall show below that $\grad(1/|\v{r}|)= - \v{r}/|\v{r}|^3$.
...
\paragraph{Diffusion:}
The idea of diffusion: for example in a gas the molecular motion effectively smooths out
the density. This can be described by the current of particles $\v{j}(\v{r})$ being
proportional to the gradient of the density $n(\v{r})$ (Fick's Law), \ie the diffusion
current flows down the concentration gradient
\[
...
The previous example on directional derivatives used the ``xyz'' notation. This gets
unwieldy for more complicated examples, and suffix notation is more convenient.
\begin{enumerate}
  \item Let $\phi(\vr) = \v{r}^2 = x_1^2 + x_2^2 + x_3^2$, then
        \[
          (\grad \v{r}^2)_i
          \,=\,
        ...
        convention to write $\v{r}^2=x_jx_j$
        \[
          (\grad \v{r}^2)_i \,=\, \partial_i\,\v{r}^2
          \,=\, \partial_i\, (x_jx_j)
        ...
        \begin{center}
          \bigbox{$
            ...
            $}
        \end{center}
        The level surfaces of $\v{r}^2$ are spheres centred on the origin, and the
        gradient of $\v{r}^2$ at $\vr$ points radially outward with magnitude $2|\v{r}|$.
  ...
  \item Let $\phi(\vr) \,=\, |\vr| \,=\, \sqrt{x_1^2 + x_2^2 + x_3^2} \,=\, (\x{j}\x{j})^{1/2}$
        \begin{eqnarray*}
          (\grad|\v{r}|)_i
          & = &
          ...
          & = &
          \frac{1}{2|\v{r}|}\; 2\, \x{i}\\[0.5ex]
          & = &
          (\v{\hat{r}})_i
          \qquad \mbox{so} \qquad
          \bigbox{$\ds \grad |\v{r}| \,=\, \frac{\v{r}}{|\v{r}|}\,=\, \v{\hat{r}}$}
        \end{eqnarray*}
        ...
        surfaces which are spheres centred on the origin.
\end{enumerate}
...
example. We shall now see what we can gain by considering more general cases. We shall
derive several \emph{identities} which hold for the gradient of any scalar field, and
which we may use to speed up the evaluation of the gradient of more complicated scalar
fields. If $\phi(\vr)$ and $\psi(\vr)$ are real scalar fields, then we have the following
properties.
\begin{enumerate}
  ...
  \item \textbf{Product rule}
        ...
        %%%     Chain rule
        %%%
  \item \textbf{Chain rule:} If $F$ is a real function of a single variable, then
        ...
        \begin{center}
          \bigbox{$\ds
            ...
            $}\,,
        \end{center}
        where $F'$ is the derivative if $F$.
...
\paragraph{Example of Chain Rule:}
If $\phir = |\vr|$ we can use result (iii) from section~(\ref{sec:examples-on-gradient})
to give
\[
  \grad F(|\vr|) \;=\;
  F'(|\vr|) \,\grad |\vr|
  ...
  F'(|\vr|)\; \hat{\v{r}}
\]
If $F\left(x\right) = x^n$, then $F'(x)=nx^{n-1}$ and we have
\begin{center}
  \bigbox{$\ds
      \grad(|\vr|^n)
      \,=\,
...
\paragraph{Example:}
Calculate $\grad\phi$ when $\ds \phi(\v{r}) = |\vr|^n \left(\v{a}\cdot\v{r}\right)^m$.
\begin{eqnarray*}
  \grad \left\{ |\vr|^n \left(\v{a}\cdot\v{r}\right)^m \right\}
  & = &
  \left( \grad |\vr|^n \right) \left(\v{a}\cdot\v{r}\right)^m
  +
  |\vr|^n \left\{ \grad \left(\v{a}\cdot\v{r}\right)^m\right\}
  ...
  %
  & = &
  n \, |\vr|^{n-2} \left(\v{a}\cdot\v{r}\right)^m \v{r}
  +
  m |\vr|^n
  \left(\v{a}\cdot\v{r}\right)^{m-1} \v{a}
\end{eqnarray*}
where, in the last line, we used
$\grad\left(\v{a}\cdot\v{r}\right)=\v{a}$ and $\grad |\vr|^n = n \, |\vr|^{n-2}
  \v{r}$.
This example demonstrates the ``toolkit'' approach to evaluating gradients of complicated
expressions. Here, we combined the product rule with known gradients of simple
scalar fields $\left(\v{a}\cdot\v{r}\right)$ and $|\vr|^n$.

\subsection{Transformation of the gradient}
We now prove that the gradient of a scalar field indeed transforms as a vector field. Let
the point $P$ have coordinates $\x{i}$ in the $\{\ei\}$ basis, and the \emph{same} point
$P$ have coordinates $\x{i}'$ in the $\{\ei\!'\}$ basis, \emph{i.e.}~we consider the
vector transformation law $x_i \to x_i' = \ell_{ij} \, x_j$. $\phi(\vr)$ is a
\emph{scalar field} if it depends only on the physical point $P$ and not on the
coordinates $\x{i}$ or $\x{i}'$ used to specify $P$. The \emph{value} of $\phi$ at $P$ is
\emph{invariant} under a change of basis, but the function may look different,
\emph{i.e.}
\begin{center}
...
Now, let us consider $\grad \phi$ in the new (primed) basis. Its components transform as
\[
...
\subsection{The operator \texorpdfstring{$\grad$}{∇}}
We can think of $\grad$ as a \emph{vector operator}, called \emph{del}, which acts on the
\emph{scalar field} $\phi(\vr)$ to produce the \emph{vector field} $\grad
  \phi(\vr)$.\medskip\\ In Cartesians coordinates: \hspace*{0.02\textwidth}
%
\bigbox{$\ds
    \grad
    \,\equiv\,
...
is a vector operator because it has the transformation properties of a vector,
\begin{center}
...
\begin{equation}
  ...
  + \mathcal{O}[(\delta\vr)^2]
  \,=\,
  \phi(\v{r}) + \delta\v{r} \cdot \grad \phi +  \mathcal{O}[(\delta\vr)^2]
  \label{eq:grad-phi}
\end{equation}
...
\begin{equation}
  \bigbox{$
      \phi(\v{r}+\v{a})
      \,=\,
      \phi(\v{r}) + \v{a} \cdot \grad \phi(\v{r}) + \mathcal{O}[(\delta\vr)^2]
    $}
  \label{eq:phi-expansion}
\end{equation}
As we shall see, this expression is very useful when $\phi(\vr)$ is
