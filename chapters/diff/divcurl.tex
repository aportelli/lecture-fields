...
We now combine the vector operator $\grad$ with a vector field to define two new
operations called \emph{divergence} and \emph{curl}. Then we define the \emph{Laplacian}.

\subsection{Divergence}

We define the \emph{divergence} of a vector field $\v{a}(\v{r})$ (sometimes abbreviated
as ``div a'') by
\begin{center}
  \fbox{
    \parbox{45mm}{
...
Indeed, under a transformation $x_i^\prime = \ell_{ij}\;x_j$, we have $\partial^\prime_i =
...
\paragraph{Example:}
If $\v{a}(\vr)=\vr$ then \quad \bigbox{ $\Div \vr ~=~ 3$} \quad which is a useful and
important result.
...
In suffix notation: \hspace*{0.07\textwidth}\parbox{0.5\textwidth}{\(\ds \Div \vr =
  \frac{\partial\x{i}}{\partial \x{i}} = \delt{ii} = 3 \)}

\paragraph{Example:} In ``xyz'' notation, let $\v{a} = x^2 z \,\v{e}_{\,x} - 2 y^3 z^2 \,\v{e}_{\,y} + x y^2 z
  \,\v{e}_{\,z}$
\begin{eqnarray*}
  \Div \v{a}
  & = &
...
\emph{pseudovector} field\footnote{Exercise: Show that if $\v{a}$ is a
...
\[
  ...
  ~=~
  \ei\; \eps{ijk}\, {\frac{\partial}{\partial x_j}} \: a_{k}
\]
...
\paragraph{Example:}
If $\v{a}(\vr)=\vr$, then~\bigbox{$ \curl \vr \,=\, 0 $}~, another useful and important
result
...
\subsection{Physical interpretation of divergence and curl}
\begin{figure}[t]
  \centering
  \includegraphics[height=5.5cm]{tikz_myradial.pdf}\qquad
  \includegraphics{tikz_v_for_rigid_body.pdf}
  \caption{Representation of a curl-free (left) and divergence-free (right) field.}
  \label{fig:curldiv_free}
\end{figure}
A full interpretation of the divergence and curl of a vector field is best left until
after we have studied the Divergence Theorem and Stokes' Theorem respectively. However,
we can gain some intuitive understanding by looking at simple examples where div and/or
curl vanish.

First consider the radial field $\v{a} = \v{r}$. We have just shown that $\Div \v{r} = 3$
and $\curl \v{r} = 0$. We may sketch the vector field $\v{a}(\v{r})$ by drawing vectors
of the appropriate direction and magnitude at selected points, as represented in
\cref{fig:curldiv_free}. These give the tangents of \emph{flow lines}. Roughly speaking,
in this example the divergence is positive because bigger arrows come out of any point
than go into it. So the field \emph{diverges}. As the field is purely radial, it
\emph{does not curl}, which is manifested by $\curl \v{r} = 0$.

Now consider the field $\v{v} = \v{\omega} \times\vr$ where $\v{\omega}$ is a
\emph{constant} vector. One can think of $\vv$ as the velocity of a point in a rigid
rotating body. \cref{fig:curldiv_free} shows a cross-section of the field $\vv$ with
$\v{\omega}$ chosen to point out of the page. We can calculate $\curl\vv$ as follows:
\begin{eqnarray*}
...
\begin{enumerate}
  \item If $\Div\,\v{a} = 0 \;\;\;$ in some region $R$, we say $\v{a}$ is \emph{divergence-free}
        or \emph{solenoidal} in $R$.
  \item If $\curl\,\v{a} = 0\;$ in some region $R$, we say $\v{a}$ is \emph{curl-free} or
        \emph{irrotational} in $R$.
\end{enumerate}
...
\[
  ...
  \lap \phi
\]
$\lap$ is the \emph{Laplacian operator}.  In Cartesian coordinates
\[
  \lap
  ~=~ \frac{\partial}{\partial \x{i}}\;
  \frac{\partial}{\partial \x{i}}
