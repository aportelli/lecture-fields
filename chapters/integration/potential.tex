...
regions, but if we have time we will explore the consequences of multiply-connected
regions.

A vector field $\v{a}(\v{r})$ is defined to be irrotational or \emph{conservative} if its
curl vanishes, \emph{i.e.} if
\[
  \curl \v{a} \,=\, 0
\]
...
\parbox{0.62\textwidth}{Let $\curl\v{a} = 0$ everywhere in a
simply-connected region, and consider two (different) paths $C_1$
and $C_2$ from point $\v{r}_{\,0}$ to point $\v{r}$, say. Applying
Stokes' theorem to the \emph{open} surface $S$ bounded by the
\emph{closed} path $C_1{-}C_2$ gives
\[
...
\]
where the $-$ve sign occurs in the second integral on the RHS because both paths are
defined to go from $\v{r}_{\,0}$ to $\v{r}$. We use $\vr'$ as integration variable to
distinguish it from the integration \emph{limits} $\vr_{\,0}$ and $\vr\,$. }
%
\hfill
%
...
Therefore, when $\curl\v{a} = 0$ everywhere in $S$, we have
\[
  \int_{C_1}\, \v{a}(\v{r}^\prime)\cdot \mbox{d}\v{r}^\prime
  \,=\,
  \int_{C_2}\, \v{a}(\v{r}^\prime)\cdot \mbox{d}\v{r}^\prime
\]
This is true for \emph{any} $S$, and therefore for \emph{any} paths $C_1$ and $C_2$ from
$\v{r}_{\,0}$ to $\v{r}$.

% Since the line integral $\ds \int\v{a}(\v{r}^\prime)\cdot
% \mbox{d}\v{r}^\prime$ is \emph{path independent} it can be a function
% \emph{only} of the \emph{end points} of the path.
...
\[
  \bigbox{$ \ds
      ...
      \int_{\ul{r}_{\,0}}^{\ul{r}} \, \v{a}(\v{r}^\prime)\cdot \mbox{d}\v{r}^\prime
...
considering two neighbouring points $\v{r}$ and $\v{r}+\mbox{d}\v{r}$, for which
\begin{eqnarray*}
  \mbox{d}\phi
  &=&
  \phi(\v{r}+\mbox{d}\v{r}) - \phi(\v{r}) \\[1ex]
  &=&
  \left[\phi(\v{r}+\mbox{d}\v{r}) - \phi(\v{r}_{\,0})\right]
  -
  \left[\phi(\v{r}) - \phi(\v{r}_{\,0})\right] \\[1ex]
  &=&
  \int_{\ul{r}_{\,0}}^{\ul{r}+ {\rm d}\ul{r}}\,
  \v{a}(\v{r}^\prime) \cdot \mbox{d}\v{r}^\prime
  \,-\,
  \int_{\ul{r}_{\,0}}^{\ul{r}}\,
  \v{a}(\v{r}^\prime)\cdot \mbox{d}\v{r}^\prime
  ...
  \mbox{d}\v{r}^\prime \,+\, O(|\mbox{d}\v{r}|^2)
  \hspace*{0.11\textwidth}
  \mbox{(\emph{choosing} the straight line from $\v{r}$ to
    $\v{r}+\mbox{d}\v{r}$)}
  \\[1ex]
  &=&
  \v{a}(\v{r})\cdot \left[ \left(\v{r} + \mbox{d}\v{r}\right) - \v{r} \right]
  \,=\,
  \v{a}(\v{r})\cdot \mbox{d}\v{r}
\end{eqnarray*}
because $\v{a}(\v{r})$ is approximately constant between $\v{r}$ and
$\v{r}+\mbox{d}\v{r}$, and the correction term $O(|\mbox{d}\v{r}|^2)$
can be ignored as $\mbox{d}\v{r}\to0$.

But $\mbox{d}\phi = \grad\phi\cdot \mbox{d}\v{r}$ (by definition), and so, since
$\mbox{d}\v{r}$ is \emph{arbitrary}, we must have
\[
...
%{\bf Terminology:}  A vector field is
%\begin{itemize}
%\vspace*{-1ex}
%\item {\bf irrotational}
%if $
%        \curl\v{a}(\v{r}) \,=\, 0 $.
%\item {\bf conservative} if $\v{a}(\v{r}) = \grad \phi\;$.

%\item For simply connected regions we have shown
%irrotational and conservative are synonymous. 
%But note that for a multiply connected region this is not the case.
%\end{itemize}
...
\begin{enumerate}
  \item $\v{r}_{\,0} = 0$:\ \ If $\phi(\v{r})$ is \emph{finite} or
        ...
        Thus $\mbox{d}\v{r}^\prime = \mbox{d}\lambda \, \v{r}\,$, and hence
        \[
          \phi(\v{r})
          \,=\,
          \int_{0}^{\ul{r}}\, \v{a}(\v{r}^\prime) \cdot \mbox{d}\v{r}^\prime
          \,=\,
          \int_{\lambda=0}^{\lambda=1} \,
          \v{a}(\lambda\v{r}) \cdot \v{r} \, \mbox{d}\lambda \,,
        \]
  ...
  \item $|\v{r}_0|=\infty$:\ \ If $\phi(\v{r})$ is \emph{finite} or
        ...
        $\mbox{d}\v{r}^\prime = \mbox{d}\lambda\, \v{r}\,$, and hence
        \[
          \phi(\v{r})
          \,=\,
          \int_{\infty}^{\ul{r}}\, \v{a}(\v{r}^\prime) \cdot \mbox{d}\v{r}'
          \,=\,
...
\paragraph{Example~1:}
Let $\v{a}(\v{r}) = (2xy+z^3)\v{e}_{\,x} + x^2\v{e}_{\,y} + 3xz^2\v{e}_{\,z}$.

First check that $\v{\nabla}\times\v{a} = 0$, so the field is conservative (exercise).
Then
\begin{eqnarray*}
  \phi(\v{r})
  &=&
  \int_{0}^{\ul{r}}\, \v{a}(\v{r}^\prime) \cdot \mbox{d}\v{r}^\prime
  \,=\,
  \int_{\,0}^1\, \v{a}(\lambda\v{r})\cdot \v{r} \, \mbox{d}\lambda \\[0.7ex]
  &=&
  \int_{\,0}^1\,
  \left[
...
%{\bf Example~1:}
%Let $\v{a}(\vr) \,=\, (\vc\cdot\v{r})\, \vc$ where $\vc$ is a constant
%vector.\\[0.5ex]
%First check that $\curl\va = 0$,
%\begin{eqnarray}
%   (\v{\nabla}\times\v{a})_i 
%    = \epsilon_{ijk}\,\partial_j(c_lx_l)c_k 
%    = \epsilon_{ijk}\,c_l\delta_{jl}c_k = \epsilon_{ijk}\,c_jc_k = 0
%                                                              \nonumber
%\end{eqnarray}
%Thus
%\begin{eqnarray*}
%        \phir
%        & = & \int_{\,0}^{\vr}\; \v{a}(\v{r'}) \cdot \v{dr}' 
%          =     \int_{\,0}^{\vr}\; (\vc\cdot\v{r'})\,\vc
%                                        \cdot\v{dr}' \\
%        & = & \int_{\,0}^1 (\vc\cdot\lambda\,\vr)\, \vc \cdot
%                (\mbox{d}\lambda \, \vr)
%          =     (\vc\cdot\v{r})^2 \, \int_{\,0}^1 \lambda\,\mbox{d}\lambda\\
%        & = &    \frac{1}{2} (\vc\cdot\v{r})^2
%\end{eqnarray*}

%Note: Always check that your $\phir$ satisfies $\v{a}(\v{r})\,=\,
%\gra\mbox{d}\,\phir$.
...
%  \left(2\, (c_l\,x_l) \, x_k + x_l \, x_l \, c_k\right) \\[0.5ex]
%  &=&
%  \epsilon_{ijk}
%  \left(
%    2  \, c_l \, \delta_{jl} \, x_k
%    + 2\, \v{c}\cdot\v{r} \, \delta_{jk}
%    + 2x_l \, \delta_{lj} \, c_k
%  \right) \\[0.5ex]
%  &=&
%  2\epsilon_{ijk}\, (c_j \, x_k + x_j \, c_k) 
%  \,=\,
%  0
%\end{eqnarray*}
Then
\begin{eqnarray*}
  \phir
  & = & \int_{\,0}^{\ul{r}}\; \v{a}(\v{r}') \cdot \mbox{d}\v{r}'
  \,=\,
  \int_0^1\; \v{a}\left(\lambda\,\vr\right)
  \cdot \left(\mbox{d}\lambda\,\vr\right)
  \\[1ex]
  & = &
  \int_0^1
  \left(
  ...
  \,+\, \lambda^2 r^2 \, \vc
  \right)
  \cdot \vr \, \mbox{d}\lambda %\\[1ex]
  ~ = ~
  %  & = &
  \left(
  2\,(\vc\cdot\v{r}) \, \vr\cdot\vr \plus r^2 \, (\vc\cdot\v{r})
  \right)
  \int_0^1 \lambda^2 \, \mbox{d}\lambda \\[1.5ex]
...
% A variation is to differentiate the first equation wrt $y$ and then
% solve for $f$, ie
% \[
%   {\partial\phi \over \partial y}
%   = {\partial \over \partial y} \int^x\, dx^\prime\, a_x(x^\prime, y, z)
%  + {\partial  f\over \partial y}(y,z) = a_y(x,y,z)
% \]
% to give a further `integration constant' $g(z)$, which can be found
% by differentiating the first equation wrt $z$.
...
%\paragraph{Example~1 revisited:}{\mbox{}
%\begin{eqnarray*}
%  \v{a}(\v{r})
%  &=&
%  (2xy+z^3, x^2, 3xz^2 ) \\[1ex]
%  &=&
%  \left(
%    \frac{\partial}{\partial x} \left(x^2y + xz^3\right), \, 
%    \frac{\partial}{\partial y} \left(x^2y + xz^3\right), \,
%    \frac{\partial}{\partial z} \left(x^2y + xz^3\right)
%  \right) \\[1ex]
%  &=&
%  \v{\nabla} \, \left(x^2y + xz^3 + \mbox{constant}\right)
%\end{eqnarray*}
...
\]
%This can be tricky to spot though.

\paragraph{Example~2 (revisited)}\mbox{}
%, let $\v{a}(\vr) \,=\, 2\,(\va\cdot\v{r})\, \vr \plus r^2 \, \va$
\[
  \v{a}(\v{r})
  \,=\,
...
%Consider again the work done by a force.
%If the force is {\it conservative}, i.e. total energy is conserved,
%then the work done is equal to minus the change in potential energy
%\[
%dV = -dW = - \vF\cdot \v{dr}= -F_{\ds i} dx_{\ds i}
%\]
%Now we can also write  dV as
%\vspace*{-2ex}
%\[
%dV = \frac{\partial V}{\partial x_{\ds i}} dx_{\ds i} = 
%(\grad V)_{\ds i} dx_{\ds i}
%\]
...
%Thus the force is minus the gradient of the (scalar) potential.  The
%minus sign is conventional and chosen so that potential energy
%decreases as the force does work.

%In this example we knew that a potential existed (we postulated
%conservation of energy). More generally we would like to know under
%what conditions can a vector field $\v{A}(\vr)$ be written as the
%gradient of a scalar field $\phi$, {\it i.e.} when does $\vA(\vr)
%\,=\, (\pm)\, \grad\,\phi(\vr)$ hold?

%{\bf Aside:} A \emph{simply connected} region $R$ is a region where
%every closed curve in $R$ can be shrunk continuously to a point while
%remaining entirely in $R$.  The inside of a sphere is simply connected
%while the region between two concentric cylinders is \emph{not} simply
%connected: it is doubly connected.
%% See {\it RHB 9.2} for discussion.
%For this course we shall be concerned with simply connected regions
%\vspace{-2ex}
...
$\v{\ddot{r}}$ (acceleration) respectively.

The particle moves under the influence of Newton's second law (N2):
\[
  m \v{\ddot r} = \vF(\vr)
\]
In time $\mbox{d}t$ the particle moves from $\v{r}$ to $\v{r}+\mbox{d}\v{r}\,$. From N2,
we get
\[
  m \, \v{\ddot r}\cdot \mbox{d}\v{r}
  \,=\,
  \vF(\vr) \cdot \mbox{d}\v{r}
  \,=\,
  -\grad V(\vr) \cdot \mbox{d}\v{r}
\]
Integrating this expression along the path of the particle starting from $\v{r}_A$ at
time $t_A\,$, to $\v{r}_B$ at time $t_B$, gives
\begin{equation}
  m \int_{\ul{r}_A}^{\ul{r}_B} \v{\ddot r}\cdot \mbox{d}\v{r}
  \,=\,
  - \int_{\ul{r}_A}^{\ul{r}_B} \grad V(\vr) \cdot \mbox{d}\v{r}
...
\[
  m \int_{\ul{r}_A}^{\ul{r}_B} \v{\ddot r} \cdot \mbox{d}\v{r}
  \,=\,
  m \int_{t_A}^{t_B} \v{\ddot r} \cdot \frac{\mbox{d} \v{r}}{dt}\, \mbox{d}t
  %  \,=\,
  %  m \int_{t_A}^{t_B} \v{\ddot r} \cdot \v{\dot r}\, \mbox{d}t
  \,=\,
  m \int_{t_A}^{t_B} \frac{1}{2} \, \frac{d}{dt} \,
  \left(\v{\dot r} \cdot \v{\dot r}\right) \mbox{d}t
  \,=\,
...
%We now return to where we started in section
%(\ref{ExamplesGravitationandElectrostatics}).
...
The \emph{gravitational field} $\vG(\vr)$ due to the mass at the origin is \emph{defined}
by
\begin{equation}
  \v{F}(\v{r})
  ~\equiv~
...
\begin{eqnarray*}
  \phir
  &=&
  -\int_{\ul{r}_0}^{\ul{r}} \v{G}(\vr') \cdot \mbox{d}\v{r}'
  \,=\,
  -\int_\infty^{1} \v{G}(\lambda\vr) \cdot \mbox{d}\lambda \v{r} \\[1ex]
  &=&
  (-)^2 \, \int_\infty^{1} \frac{Gm\, (\v{r}\cdot\vr)}{r^3} \:
  \frac{\mbox{d}\lambda}{\lambda^2}
  \,=\,
  -\frac{Gm}{r}
\end{eqnarray*}

\textbf{Note:} In this example, the vector field $\v{G}$ is
\emph{singular} at the origin $\vr = 0$. This implies that we have to
exclude the origin,
%(but since it is a single point our domain is still simply 
%connected). 
so it's not possible to obtain the scalar potential at $\vr$ by integration along a path
from the origin. Instead we integrate from infinity, which in turn means that the
gravitational potential at infinity is zero.

\textbf{Note:} Since $\vF = m_1 \, \v{G} = -\grad(m_1 \, \phi)$, the
\emph{potential energy} of the mass $m_1$ is $V(\v{r})= m_1
  \phi(\v{r})$. The distinction (a convention) between potential and
potential energy is a common source of confusion.
...
taken as small, so as not to disturb the electric field.

The \emph{electrostatic potential} may be obtained by inspection, or by integrating $E=
  -\grad \phi$ from infinity to $\vr$,
\begin{equation}
...
% \rho(\v{r})\, \mbox{d}V$ is the total charge in volume $V$.
% 
...
Example: show that $\Div\curl\v{a} \equiv 0$ independent of co-ordinate system:
...
Now use Stokes' theorem to write
\[
  \intS \left(\curl\v{a}\right)\cdot\dd\v{S}
  \,=\,
  ...
  \,=\,
  \ointC \v{a}\cdot \mbox{d}\v{r} \,-\, \ointC \v{a}\cdot \mbox{d}\v{r}
  \,=\,
  0
\]
...
the right hand sense with respect to the outward normal of the surface.)

Since this result holds for arbitrary volumes, we must have
\[
  \Div\curl\v{a} \,\equiv\, 0
\]
...
potential for this force.\footnote{This isn't quite true - we \emph{can} construct a
  scalar potential $\tilde{V}$ for the Lorentz force, but it's a function of both $\v{r}$
  \emph{and} $\v{v}$, and the expression for the force takes the form $\v{F} = -\grad
    \tilde{V}\ + $ another term. See \emph{Lagrangian Dynamics} in Junior Honours.}

It is often said that ``a magnetic force does no work'' because
\[
  \v{F} \cdot \dd \v{r}
  \,=\,
...
any explicit dependence on $\vr$.

However forces that depend only on $\vr$ are non-conservative when $\curl \vF(\vr) \neq
  0\,$.

Generally, one may write \emph{any} vector field $\va(\v{r})$ as
\[
  \va \,=\, -\del \phi \,+\, \curl \vA \,,
\]
...
conservative) part $-\del \phi$ and a divergence-free part $\curl \vA$. We are not yet in
a position yet to prove this theorem.
